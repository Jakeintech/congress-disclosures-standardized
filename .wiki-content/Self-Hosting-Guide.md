...
Get your free API key from: https://api.congress.gov/sign-up/
...
# Re-process failed messages (after fixing root cause)
...
aws lambda invoke \
  ...
  --payload '{"Records": [{"body": "{\"doc_id\": \"8221216\", \"year\": 2025}"}]}' \
  --log-type Tail \
  response.json
...
- Use pypdf + Tesseract instead of Textract ($0 vs $13.50/month)
...
2. **Set Up Scheduled Ingestion**:
   ...
   - See [[Running-Pipelines]] for automation

3. **Build Gold Layer**:
   - Implement additional transformations
   - Create query-facing tables
   - Set up public API (see [[API-Documentation]])
