...
def check_dim_members():
    print(f"Checking dim_members schema in bucket: {S3_BUCKET}")
    ...
    path = f"s3://{S3_BUCKET}/gold/dimensions/dim_members/part-0000.parquet"
    print(f"Reading: {path}")
    
    try:
        s3 = boto3.client('s3')
        resp = s3.list_objects_v2(Bucket=S3_BUCKET, Prefix='gold/dimensions/dim_members/', MaxKeys=1)
