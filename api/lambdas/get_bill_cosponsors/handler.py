...
def handler(event, context):
    ...
    try:
        ...
        api_url = f"{CONGRESS_API_BASE}/bill/{congress}/{bill_type}/{bill_number}/cosponsors"
        ...
        try:
            ...
            if resp.status_code == 404:
                return error_response(message="Bill cosponsors not found", status_code=404)

            if resp.status_code != 200:
                logger.error(f"Congress.gov API error: {resp.status_code} - {resp.text}")
                return error_response(message="Failed to fetch bill cosponsors from Congress.gov", status_code=502)
            ...
            for item in items:
                cleaned_items.append({
                    'bioguideId': item.get('bioguideId'),
                    'name': item.get('firstName', '') + ' ' + item.get('lastName', ''),
                    'state': item.get('state'),
                    'party': item.get('party'),
                    'district': item.get('district'),
                    'sponsorshipDate': item.get('sponsorshipDate'),
                    'isOriginalCosponsor': item.get('isOriginalCosponsor')
                })

            return success_response({
                'bill_id': bill_id,
                'cosponsors': cleaned_items,
            ...
            return error_response(message="Failed to fetch bill cosponsors", status_code=502)

    except Exception as e:
        logger.error(f"Error fetching bill cosponsors: {e}", exc_info=True)
