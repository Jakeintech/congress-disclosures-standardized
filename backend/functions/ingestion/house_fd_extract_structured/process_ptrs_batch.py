#!/usr/bin/env python3
"""Process PTRs locally using Lambda handler logic."""

import sys
import os
from pathlib import Path

# Add lib to path (via symlink in this directory)
...
# Get AWS config from Terraform outputs
def get_aws_config():
    try:
        ...
        return {
            's3_bucket_id': outputs.get('s3_bucket_id', {}).get('value'),
            's3_region': outputs.get('s3_region', {}).get('value', 'us-east-1'),
        }
    except:
        # Fallback to hardcoded value
        return {
            's3_bucket_id': 'congress-disclosures-standardized',
            's3_region': 'us-east-1'
        }
    ...
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as tmp_pdf:
        ...
        try:
            ...
            try:
                ...
                # Download from House (PTRs use ptr-pdfs path)
