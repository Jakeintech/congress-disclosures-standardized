...
def handler(event, context):
    ...
    try:
        ...
        api_url = f"{CONGRESS_API_BASE}/bill/{congress}/{bill_type}/{bill_number}/subjects"
        ...
        try:
            ...
            if resp.status_code == 404:
                return error_response(message="Bill subjects not found", status_code=404)

            if resp.status_code != 200:
                logger.error(f"Congress.gov API error: {resp.status_code} - {resp.text}")
                return error_response(message="Failed to fetch bill subjects from Congress.gov", status_code=502)
            ...
            return error_response(message="Failed to fetch bill subjects", status_code=502)

    except Exception as e:
        logger.error(f"Error fetching bill subjects: {e}", exc_info=True)
