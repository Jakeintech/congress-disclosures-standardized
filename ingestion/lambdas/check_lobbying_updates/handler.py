...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Schedule main filing ingest if we found new quarters
        if has_new_filings:
            ingest_tasks.append(
                {"filing_year": year, "filing_type": "FILING", "entity_type": None}
            )
            ...
            # STORY-UPDATE: Ensure we capture ALL reference data
            # (Lobbyists, Registrants, Clients)
            # These endpoints don't strictly depend on year but good to
            # refresh periodically.
            # We pass year just to satisfy the strict ItemSelector,
            # though it might be ignored by the logic.
            ingest_tasks.append(
                {"filing_year": year, "filing_type": None, "entity_type": "LOBBYIST"}
            )
            ingest_tasks.append(
                {"filing_year": year, "filing_type": None, "entity_type": "REGISTRANT"}
            )
            ingest_tasks.append(
                {"filing_year": year, "filing_type": None, "entity_type": "CLIENT"}
            )
