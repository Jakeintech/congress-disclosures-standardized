...
export function ConflictDetectionCard({
    ...
    const { data, isLoading, error, refetch } = useConflicts(severity, limit);
    ...
    const getSeverityBadge = (sev: string) => {
        ...
        return (
            <Badge className={variants[s] || 'bg-gray-500'}>
                {s}
            </Badge>
        );
    };
                            ...
                            {showSummary && summary && (
                                <div className="grid grid-cols-4 gap-2 mb-4">
                                    <div className="text-center p-2 rounded bg-red-50 dark:bg-red-950">
                                        <div className="text-2xl font-bold text-red-600">{summary.critical_count || 0}</div>
                                    ...
                                    <div className="text-center p-2 rounded bg-yellow-50 dark:bg-yellow-950">
                                        <div className="text-2xl font-bold text-yellow-600">{summary.medium_count || 0}</div>
                                        <div className="text-xs text-muted-foreground">Medium</div>
                                    </div>
                                    <div className="text-center p-2 rounded bg-blue-50 dark:bg-blue-950">
                                        <div className="text-2xl font-bold text-blue-500">{summary.low_count || 0}</div>
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span className="text-sm text-muted-foreground">
                                                                Score: {formatScore(conflict.conflict_score || 0)}
                                                            </span>
                                                        </div>
                                                        <div className="text-sm mt-1">
                                                            <span className="font-mono bg-muted px-1 rounded">{conflict.ticker}</span>
                                                            <span className="mx-2">•</span>
                                                            <span>{conflict.transaction_type || 'TRADE'}</span>
                                                        ...
                                                        <div className="text-xs text-muted-foreground">
                                                            Trade: {conflict.trade_date || 'N/A'} •
                                                            {Math.abs(conflict.days_offset || 0)} days {(conflict.days_offset || 0) > 0 ? 'before' : 'after'} bill action
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
